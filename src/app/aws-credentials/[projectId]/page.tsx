...
// AWS Credentials onboarding page for a project
export default function AWSCredentialsPage() {
  ...
  // Project is considered complete if all onboarding steps are done
  const isProjectComplete = Boolean(project && project.csp && project.profile && project.repoUrl);

  /**
   * Handles AWS credentials form submission.
   * Validates input, dispatches configureAwsProfileThunk, and shows feedback.
   */
  const handleSaveCredentials = async () => {
    if (isProjectComplete) return;

    // Validation patterns for AWS keys
    ...
    try {
      // Dispatch thunk to configure AWS profile
      ...
      // Redirect to GitHub repo step
      router.push(`/github-repo/${projectId}`);
    } catch (err: unknown) {
      // Handle backend errors and show user-friendly feedback
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="w-full pt-15 pb-15 bg-[#000000]">
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {/* Save Button and Project Info Preview */}
